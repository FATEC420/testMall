        ...
        <el-table-column type="expand">
          <template slot-scope="scope">
            <el-row
              ...
              v-for="(it1, i1) in scope.row.children"
              :key="it1.id"
            >
              <!-- 渲染一级权限 -->
              <el-col :span="5">
                <el-tag closable @close="removeRightById(scope.row, it1.id)">
                  {{ it1.authName }}
                </el-tag>
                ...
                >
                  ...
                  <el-col :span="13">
                    <el-tag
                      ...
                      type="warning"
                      closable
                      @close="removeRightById(scope.row, it3.id)"
                    >
                      {{ it3.authName }}
                    </el-tag>
                  </el-col>
...
export default {
  ...
  methods: {
    ...
    //通过递归的形式获取角色下所有三级权限id，并保存在defKeys中
    getLeafKeys(node, arr) {
      //如果当前node结点不包含，则是三级结点
      if (!node.children) {
